...
# ── 1b. Clear previous-run artifacts ─────────────────────────────────────────
# Every file below is regenerated by this script. Leaving them is not merely
# untidy, it is a correctness bug: the dmg name is version-independent
# (artifactName "${productName}-${arch}.${ext}") so it overwrites cleanly and
# the one-dmg check at step 3 never trips, but the ZIP carries the version
# ("SkinnyVideo-3.0.1-arm64-mac.zip"), so a version bump ADDS a second zip
# instead of replacing it. `ZIP=$(find ... -name '*-mac.zip')` at step 7 then
# holds two newline-separated paths, `[ ! -f "$ZIP" ]` becomes a two-argument
# test, and the run dies AFTER notarization and stapling have been spent.
#
# Named globs only — never `rm -rf dist`. dist/mac-arm64/ (the unpacked .app)
# is left alone: electron-builder replaces it wholesale, and blowing away a
# directory tree is exactly the blast radius this list avoids.
#
# Deliberately NOT removed: dist/corresponding-source/*.tar.xz and *.tar.gz.
# Those are sha256-pinned downloads that fetch-corresponding-source.sh
# re-verifies and reuses as a cache; deleting them re-downloads ~13 MB for no
# benefit. Their SHA256SUMS IS removed — the fetch script truncates it with a
# `>` redirect so it cannot double up, but clearing it keeps this list honest
# about everything the run rewrites.
#
# This runs AFTER the credentials gate above on purpose: an abort for missing
# credentials must leave the previous release's artifacts intact, which is the
# promise made in this file's header.
echo "release: clearing previous-run artifacts from $DIST_DIR/"
removed=$(ls -1 \
  "$DIST_DIR"/*.dmg \
  "$DIST_DIR"/*.dmg.blockmap \
  "$DIST_DIR"/*-mac.zip \
  "$DIST_DIR"/*-mac.zip.blockmap \
  "$DIST_DIR"/latest-mac.yml \
  "$DIST_DIR"/builder-debug.yml \
  "$DIST_DIR"/corresponding-source/SHA256SUMS \
  2>/dev/null | tr '\n' ' ')
rm -f \
  "$DIST_DIR"/*.dmg \
  "$DIST_DIR"/*.dmg.blockmap \
...
# ── 3b. Sign the dmg CONTAINER ───────────────────────────────────────────────
# electron-builder signs the .app inside the image; the disk image itself is
# left unsigned (build.dmg.sign is unset, and stays unset — this is the one
# place the container is signed, so there is no second code path to keep in
# step). An unsigned-but-notarized dmg passes Gatekeeper on download because
# the stapled ticket vouches for it, but `spctl --assess --type open
# --context context:primary-signature` rejects it, and anything that copies
# the image without the quarantine bit loses the only thing attesting to it.
#
# Order matters and is asserted by the steps below: sign -> notarize ->
# staple -> blockmap -> feed rewrite. Signing rewrites the dmg bytes, so it
# MUST precede the notarization submit; stapling rewrites them again, so the
# blockmap and latest-mac.yml are regenerated after that.
#
# Identity is spelled out rather than left to keychain auto-discovery: this
# machine holds exactly one Developer ID, but a script that silently picks
# "whatever is in the keychain" signs a release with the wrong cert the day
# that stops being true.
SIGN_ID="Developer ID Application: Pritam Malakar (5Q58R4CVQY)"
echo "release: signing the dmg container as $SIGN_ID"
codesign --sign "$SIGN_ID" --timestamp --force "$DMG"
